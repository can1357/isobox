...
func linuxRuntimePathCandidates(s Spec) []string {
	...
	if len(s.Args) > 0 {
		arg0 := s.Args[0]
		if hasPathSyntax(arg0) {
			paths = append(paths, cleanExplicitPath(arg0))
		} else if arg0 != "" {
	...
	for _, p := range paths {
		...
		if hasPathSyntax(p) {
			if _, err := os.Lstat(p); err == nil {
				...
				clean = cleanExplicitPath(p)
			}
		} else {
			clean = canonPath(p)
		}
		clean = linuxPathClean(clean)
...
func pathInLinuxScope(path string, scopes []string) bool {
	...
	clean := linuxPathClean(path)
	for _, scope := range scopes {
		scope = linuxPathClean(scope)
		if scope == "/" || clean == scope {
			return true
		}
		if strings.HasPrefix(clean, scope+"/") {
			return true
		}
	}
	return false
